...
class AgentLoop:
    ...
    def __init__(
        ...
        try:
            ...
            try:
                builtin_path = Path(str(importlib.resources.files("swarm"))).parent / "skills_builtin"
            except Exception:
                builtin_path = Path(__file__).parent.parent.parent / "skills_builtin"
    ...
    async def _get_dream_consolidator(self, threshold: int):
        ...
        if self._dream_consolidator is None:
            ...
            from swarm.config.paths import chroma_dir

...
class TestRequestContext:
    def test_immutable(self):
        ctx = RequestContext(
            trace_id="t1", chat_id="chat_A", chat_type="p2p", user_id="user_1", message_id="msg_1"
        )
...
class TestAgentRunner:
    ...
    async def test_simple_response(self):
        ...
        provider.chat = AsyncMock(
            return_value=LLMResponse(content="Hello!", stop_reason="end_turn")
        )
...
class TestAgentLoop:
    ...
    async def test_direct_process(self, mock_provider):
        ...
        mock_provider.chat = AsyncMock(
            return_value=LLMResponse(content="Hi there!", stop_reason="end_turn")
        )
    ...
    async def test_concurrent_different_sessions(self, mock_provider):
        ...
        mock_provider.chat = AsyncMock(
            side_effect=[LLMResponse(content=r, stop_reason="end_turn") for r in responses]
        )
        loop = AgentLoop(bus=bus, provider=mock_provider, workspace=".")
